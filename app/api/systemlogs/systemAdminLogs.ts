...
async function fetchCurrentDbSize(): Promise<number> {
  const { data, error } = await supabaseAdmin
    .from(__schema.public.dbSizeView)
    .select("*")
    .single();
...
export async function smartUpsertUsage(): Promise<number> {
  const [dbSize, latestLog] = await Promise.all([
    fetchCurrentDbSize(),
    getLatestUsageLog(),
  ]);
  ...
  if (latestLog) {
    ...
    if (isExpired) {
      ...
      const { error } = await supabaseAdmin
        ...
        .from(__schema.admin.usageLog)
        .upsert(
          { hour_bucket: currentHourBucket, db_size: dbSize, updated_at: now.toISOString() },
      ...
      const { error } = await supabaseAdmin
        .schema(__schema.admin.schema)
        .from(__schema.admin.usageLog)
        .update({ db_size: dbSize, updated_at: now.toISOString() })
        .eq("id", latestLog.id);
      if (error) throw error;
    }
  } else {
    ...
    const { error } = await supabaseAdmin
      ...
      .from(__schema.admin.usageLog)
      .upsert(
        { hour_bucket: currentHourBucket, db_size: dbSize },
        { onConflict: "hour_bucket" }
      );
    if (error) throw error;
  }
