...
import { ADMIN_AUTH } from '@/app/login/auth'
  ...
  useEffect(() => {
    if (phase === 'username' || phase === 'password') {
      setTimeout(() => {
        setLines((prev) => [
          ...prev,
          { text: phase === 'username' ? 'login: ' : 'password: ', type: 'prompt' },
        ])
      ...
      setTimeout(
        () =>
          setLines((p) => [
            ...p,
            { text: '', type: 'system' },
            { text: '[auth] verifying credentials...', type: 'system' },
          ]),
        500,
      )
      ...
      if (isValid) {
        ...
        setTimeout(
          () =>
            setLines((p) => [
              ...p,
              { text: '[auth] identity confirmed', type: 'success' },
              { text: 'ACCESS GRANTED — redirecting...', type: 'success' },
            ]),
          1500,
        )
        setTimeout(() => {
        ...
        setTimeout(
          () =>
            setLines((p) => [
              ...p,
              { text: '[auth] credential mismatch detected', type: 'error' },
              { text: 'ACCESS DENIED — invalid credentials.', type: 'error' },
              { text: '', type: 'system' },
            ]),
          1500,
        )
  ...
  return (
    <div className="flex h-full flex-col" onClick={() => inputRef.current?.focus()}>
      ...
      {(phase === 'username' || phase === 'password') && (
        <form onSubmit={handleSubmit} className="mt-0.5 flex items-center">
          <span className="text-foreground mr-2 font-semibold">❯</span>
          <input
            ref={inputRef}
            type={phase === 'password' ? 'password' : 'text'}
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            className="text-foreground flex-1 border-none bg-transparent font-mono text-sm caret-transparent outline-none"
            autoFocus
            spellCheck={false}
          />
          <span
            className={`font-bold text-emerald-400 ${cursorVisible ? 'opacity-100' : 'opacity-0'}`}
          >
            ▎
          </span>
        </form>
      ...
      {/* Checking animation */}
      {phase === 'checking' && (
        <div className="text-muted-foreground mt-2 flex animate-pulse gap-1">
          <span>·</span>
          <span>·</span>
          <span>·</span>
        </div>
